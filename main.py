...
def flush_whale_alerts():
    ...
    В сводке показываются ТОЛЬКО buy/sell — transfer (кошелёк-кошелёк) слишком шумный
    и не несёт сигнала покупки/продажи, поэтому не выводится, хотя продолжает
    учитываться внутри для сортировки монет по общему объёму.
