...
if __name__ == "__main__":
    ...
    app.run(host="0.0.0.0", port=port)# Явная связка thread_id -> tf-ключ (как в TF_TO_THREAD), нужна чтобы фильтровать closed_trades по tf
THREAD_TO_TF = {
    THREAD_1H:  "60",
    THREAD_4H:  "240",
    THREAD_12H: "720",
...
def send_whale_alert(coin, side, price, size, value):
    ...
    entry = (
        "&#128011; <b>" + coin + "</b> (биржа) " + emoji + " " + side_ru
        + ": $" + "{:,.0f}".format(value)
        + "  (цена $" + "{:.6g}".format(price) + ", объём " + "{:.4g}".format(size) + " " + coin + ")"
    )
    with whale_alert_lock:
